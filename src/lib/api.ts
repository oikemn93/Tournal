...
export async function loadBoutiqueSnapshot<T>(boutiqueId: string, options: BoutiqueSnapshotOptions = {}): Promise<T | null> {
    ...
    const [boutiques, categories, products, entries, clients, suppliers, invoices, payments, advances, creditRefunds, charges, sessions, auditLogs, userScope] = await Promise.all([
      ...
      dataRequest<any[]>(`invoice_payments?select=*${scoped()}${paymentWindow}&order=paid_at.asc`), (options.historyOnly ? Promise.resolve([]) : dataRequest<any[]>(`client_advances?select=*${scoped()}&or=(paid_at.gte.${historyFromFilter},and(allocated_amount.not.is.null,amount.not.is.null,allocated_amount.lt.amount))&order=paid_at.desc,id.desc`)),
...
export type RelationalTransfer = {
  ...
  stock_transfer_lines:Array<{ id:number; source_product_id:number; destination_product_id:number|null; product_name:string; unit:string; qty:number; prix_unit:number; sell_unit:string|null; sell_qty:number|null }>;
};
export async function getStockTransfers(boutiqueId: string) {
  return dataRequest<RelationalTransfer[]>(`stock_transfers?select=id,from_boutique_id,to_boutique_id,status,relationship_type,total_amount,invoice_id,charge_id,note,created_at,stock_transfer_lines(id,source_product_id,destination_product_id,product_name,unit,qty,prix_unit,sell_unit,sell_qty)&or=(from_boutique_id.eq.${encodeURIComponent(boutiqueId)},to_boutique_id.eq.${encodeURIComponent(boutiqueId)})&order=created_at.desc`);
}
export async function createStockTransfer(params: { fromBoutiqueId:string; toBoutiqueId:string; lines:Array<{productId:number;qty:number;unitPrice:number;sellUnit?:string;sellQty?:number}>; note?:string }) {
  return dataRequest<{transfer_id:string;status:string;relationship_type:"same_owner"|"commercial";total_amount:number}>("rpc/create_stock_transfer", { method:"POST", body:JSON.stringify({ p_from_boutique_id:params.fromBoutiqueId,p_to_boutique_id:params.toBoutiqueId,p_idempotency_key:crypto.randomUUID(),p_lines:params.lines.map(line=>({product_id:line.productId,qty:line.qty,unit_price:line.unitPrice,sell_unit:line.sellUnit ?? null,sell_qty:line.sellQty ?? null})),p_note:params.note ?? null }) });
}
export async function acceptStockTransfer(transferId: string, mappings: Array<{transferLineId:number;destinationProductId?:number;createNew?:boolean;newName?:string;categoryId?:string|null;piecesPerLot?:number;lengthPerPiece?:number;sellUnit?:string}> = []) {
  return dataRequest<{transfer_id:string;status:string;relationship_type:"same_owner"|"commercial";total_amount:number;invoice_id:string|null;charge_id:number|null;supplier_id?:number|null}>("rpc/accept_stock_transfer_custom", { method:"POST", body:JSON.stringify({ p_transfer_id:transferId,p_idempotency_key:crypto.randomUUID(),p_line_mappings:mappings.map(mapping=>({transfer_line_id:mapping.transferLineId,destination_product_id:mapping.destinationProductId ?? null,create_new:mapping.createNew === true,new_name:mapping.newName ?? null,category_id:mapping.categoryId ?? null,pieces_per_lot:mapping.piecesPerLot ?? null,length_per_piece:mapping.lengthPerPiece ?? null,sell_unit:mapping.sellUnit ?? null})) }) });
}
