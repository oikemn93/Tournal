...
export function ComptabiliteView({ boutique, canSeeMargin = false }: { boutique: Boutique; canSeeMargin?: boolean }) {
  ...
  return <div data-screen-source="canonical-report-v7-ui" className="space-y-4 pb-24">
    ...
    <div className="space-y-2">
      ...
      <Accordion title="Stock" subtitle="Où mon capital est-il immobilisé ?" icon={<Boxes size={17}/>} active={open==="stock"} onClick={()=>void toggle("stock")} color={RC}>{sectionLoading==="stock"?<Loading/>:sectionErrors.stock?<SectionError message={sectionErrors.stock}/>:<StockReportSection report={stock} days={dormantDays} onDays={d=>void refreshStock(d)} canSeeMargin={canSeeMargin}/>}</Accordion>
