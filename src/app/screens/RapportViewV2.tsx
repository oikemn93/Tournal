import type { ReportPageRequest } from "./ReportPage";
...
export function ComptabiliteView({ boutique, canSeeMargin=false, comparisonBoutiques=[] }: { boutique:Boutique; canSeeMargin?:boolean; comparisonBoutiques?:Boutique[] }) {
  ...
  const [financeSales,setFinanceSales]=useState<SalesProductReport|null>(null);
  ...
  const [pageRequest,setPageRequest]=useState<ReportPageRequest>({offset:0,sort:"revenue",direction:"desc"});
  ...
  const filterKey=useMemo(()=>JSON.stringify([filters.categoryId??null,filters.operatorId??null,filters.paymentMethod??null,filters.clientType??null,filters.entityKind??null,filters.entityId??null]),[filters.categoryId,filters.operatorId,filters.paymentMethod,filters.clientType,filters.entityKind,filters.entityId]);
  ...
  useEffect(()=>{let cancelled=false;reportGenerationRef.current+=1;for(const controller of Object.values(sectionControllersRef.current))controller?.abort();sectionControllersRef.current={};setSectionLoading(null);setSectionErrors({});setError("");setPageRequest({offset:0,sort:"revenue",direction:"desc"});setOpen("sales");setSales(null);setTeam(null);setStock(null);setClients(null);setCharges(null);setFinanceSales(null);setMetrics(null);setPrevious(null);setMetricsLoading(true);Promise.all([loadFinancialMetrics({boutiqueId:boutique.id,...bounds}),loadFinancialMetrics({boutiqueId:boutique.id,...prevBounds})]).then(([a,b])=>{if(!cancelled){setMetrics(a);setPrevious(b);}}).catch(cause=>{if(!cancelled)setError(cause instanceof Error?cause.message:"Rapport indisponible");}).finally(()=>{if(!cancelled)setMetricsLoading(false);});return()=>{cancelled=true;for(const controller of Object.values(sectionControllersRef.current))controller?.abort();};},[boutique.id,bounds.from,bounds.to,prevBounds.from,prevBounds.to]);

  useEffect(()=>{const controller=new AbortController();setFilterOptions(null);loadReportFilterOptions({boutiqueId:boutique.id,...bounds,signal:controller.signal}).then(result=>{if(!controller.signal.aborted)setFilterOptions(result);}).catch(cause=>{if(!controller.signal.aborted&&!(cause instanceof DOMException&&cause.name==="AbortError"))setError("Filtres du rapport indisponibles");});return()=>controller.abort();},[boutique.id,bounds.from,bounds.to]);

  useEffect(()=>{reportGenerationRef.current+=1;for(const controller of Object.values(sectionControllersRef.current))controller?.abort();sectionControllersRef.current={};setSectionLoading(null);setSectionErrors({});setSales(null);setTeam(null);setStock(null);setClients(null);setPageRequest({offset:0,sort:"revenue",direction:"desc"});},[filterKey]);

  useEffect(()=>{const q=searchQuery.trim();if(q.length<2){setSearchHits([]);setSearchLoading(false);return;}const controller=new AbortController();setSearchHits([]);setSearchLoading(true);const timer=window.setTimeout(()=>{setSearchLoading(true);searchReportEntities({boutiqueId:boutique.id,...bounds,query:q,filters,signal:controller.signal}).then(result=>{if(!controller.signal.aborted)setSearchHits((result.hits??[]).filter(hit=>canSeeMargin||hit.kind!=="employee"));}).catch(cause=>{if(!controller.signal.aborted&&!(cause instanceof DOMException&&cause.name==="AbortError"))setSearchHits([]);}).finally(()=>{if(!controller.signal.aborted)setSearchLoading(false);});},250);return()=>{window.clearTimeout(timer);controller.abort();};},[boutique.id,bounds.from,bounds.to,searchQuery,filterKey,canSeeMargin]);
  ...
  function sectionReady(section:Section){if(section==="finance")return Boolean(financeSales);if(section==="sales")return Boolean(sales);if(section==="team")return Boolean(team);if(section==="stock")return Boolean(stock);if(section==="clients")return Boolean(clients);if(section==="charges")return Boolean(charges);return true;}
  async function toggle(section:Section){if(!sections.some(item=>item.id===section))return;const requestedPage=section===open?pageRequest:{offset:0,sort:"revenue",direction:"desc" as const};if(section!==open)setPageRequest(requestedPage);abortSectionRequests();setSectionLoading(null);setOpen(section);if(section==="group"||sectionReady(section))return;const generation=reportGenerationRef.current;const controller=new AbortController();sectionControllersRef.current[section]=controller;try{setSectionLoading(section);setSectionErrors(current=>({...current,[section]:undefined}));const filtered=hasReportFilters(filters);if(section==="sales"){const result=filtered?await loadFilteredSalesProductReport({boutiqueId:boutique.id,...bounds,filters,page:requestedPage,signal:controller.signal}):await loadSalesProductReport({boutiqueId:boutique.id,...bounds,page:requestedPage,signal:controller.signal});if(!controller.signal.aborted&&generation===reportGenerationRef.current)setSales(result);}if(section==="team"){const result=filtered?await loadFilteredEmployeePerformanceReport({boutiqueId:boutique.id,...bounds,filters,page:requestedPage,signal:controller.signal}):await loadEmployeePerformanceReport({boutiqueId:boutique.id,...bounds,page:requestedPage,signal:controller.signal});if(!controller.signal.aborted&&generation===reportGenerationRef.current)setTeam(result);}if(section==="stock"){const result=filtered?await loadFilteredStockInventoryReport({boutiqueId:boutique.id,...bounds,dormantDays,filters,page:requestedPage,signal:controller.signal}):await loadStockInventoryReport({boutiqueId:boutique.id,...bounds,dormantDays,page:requestedPage,signal:controller.signal});if(!controller.signal.aborted&&generation===reportGenerationRef.current)setStock(result);}if(section==="clients"){const result=filtered?await loadFilteredClientReport({boutiqueId:boutique.id,...bounds,filters,page:requestedPage,signal:controller.signal}):await loadClientReport({boutiqueId:boutique.id,...bounds,page:requestedPage,signal:controller.signal});if(!controller.signal.aborted&&generation===reportGenerationRef.current)setClients(result);}if(section==="charges"){const result=await loadChargeReport({boutiqueId:boutique.id,...bounds,signal:controller.signal});if(!controller.signal.aborted&&generation===reportGenerationRef.current)setCharges(result);}if(section==="finance"){const result=await loadSalesProductReport({boutiqueId:boutique.id,...bounds,signal:controller.signal});if(!controller.signal.aborted&&generation===reportGenerationRef.current)setFinanceSales(result);}}catch(cause){if(!(cause instanceof DOMException&&cause.name==="AbortError")&&!controller.signal.aborted&&generation===reportGenerationRef.current)setSectionErrors(current=>({...current,[section]:"Impossible de charger ce rapport, réessayer"}));}finally{if(sectionControllersRef.current[section]===controller)delete sectionControllersRef.current[section];if(!controller.signal.aborted&&generation===reportGenerationRef.current)setSectionLoading(current=>current===section?null:current);}}
  async function refreshStock(days:number){setDormantDays(days);if(open!=="stock")return;sectionControllersRef.current.stock?.abort();const generation=reportGenerationRef.current;const controller=new AbortController();sectionControllersRef.current.stock=controller;setSectionLoading("stock");setSectionErrors(current=>({...current,stock:undefined}));try{const result=hasReportFilters(filters)?await loadFilteredStockInventoryReport({boutiqueId:boutique.id,...bounds,dormantDays:days,filters,page:pageRequest,signal:controller.signal}):await loadStockInventoryReport({boutiqueId:boutique.id,...bounds,dormantDays:days,page:pageRequest,signal:controller.signal});if(!controller.signal.aborted&&generation===reportGenerationRef.current)setStock(result);}catch(cause){if(!(cause instanceof DOMException&&cause.name==="AbortError")&&!controller.signal.aborted&&generation===reportGenerationRef.current)setSectionErrors(current=>({...current,stock:"Impossible de charger ce rapport, réessayer"}));}finally{if(sectionControllersRef.current.stock===controller)delete sectionControllersRef.current.stock;if(!controller.signal.aborted&&generation===reportGenerationRef.current)setSectionLoading(current=>current==="stock"?null:current);}}

  useEffect(()=>{if(!metrics||sectionLoading||sectionErrors[open]||sectionReady(open)||open==="group")return;void toggle(open);},[metrics,open,sales,team,stock,clients,charges,sectionLoading,sectionErrors,filterKey,pageRequest,financeSales]);
  ...
  function openSearchHit(hit:ReportSearchHit){if(hit.kind==="employee"&&!canSeeMargin)return;setSearchQuery("");setSearchHits([]);setFilters(current=>({...current,entityKind:hit.kind,entityId:hit.id||"__unassigned__",entityLabel:hit.label}));setOpen(hit.section);}
  function changePage(request:ReportPageRequest){abortSectionRequests();reportGenerationRef.current+=1;setSectionLoading(null);setSectionErrors({});setSales(null);setTeam(null);setStock(null);setClients(null);setPageRequest(request);}
  ...
  return <div data-screen-source="canonical-report-v10-filters" className="space-y-4 pb-24">
    ...
    {activeSection&&<section id={`report-panel-${activeSection.id}`} role="tabpanel" aria-labelledby={`report-tab-${activeSection.id}`} data-report-active-section={activeSection.id} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm"><div className="flex items-center gap-3 border-b border-border px-4 py-4"><span style={{color:RC}}>{activeSection.icon}</span><div><h2 className="text-base font-black">{activeSection.label}</h2><p className="text-xs text-muted-foreground">{activeSection.subtitle}</p></div></div><div className="p-4">{hasReportFilters(filters)&&["finance","charges","group"].includes(open)&&<p className="mb-4 rounded-lg bg-muted p-3 text-xs">Ce rapport conserve le périmètre global de la période ; les filtres commerciaux concernent Ventes, Équipe, Stock et Clients.</p>}{open==="sales"&&body("sales",<SalesReportSection report={sales} onPage={changePage} metrics={metrics} canSeeMargin={canSeeMargin}/>)}{open==="team"&&canSeeMargin&&body("team",<TeamReportSection report={team} onPage={changePage}/>)}{open==="stock"&&body("stock",<StockReportSection report={stock} onPage={changePage} days={dormantDays} onDays={days=>void refreshStock(days)} canSeeMargin={canSeeMargin}/>)}{open==="clients"&&body("clients",<ClientReportSection report={clients} onPage={changePage} color={RC}/>)}{open==="charges"&&body("charges",<ChargesReportSection report={charges} metrics={metrics}/>)}{open==="finance"&&body("finance",<FinancialDeepReportSection metrics={metrics} salesReport={financeSales} canSeeMargin={canSeeMargin} color={RC}/>)}{open==="group"&&groupBoutiques.length>1&&<MultiBoutiqueReportSection boutiques={groupBoutiques} from={bounds.from} to={bounds.to}/>}</div></section>}
