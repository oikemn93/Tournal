...
export function ComptabiliteView({ boutique, canSeeMargin = false }: { boutique: Boutique; canSeeMargin?: boolean }) {
  ...
  return <div data-screen-source="canonical-report-v6-ui" className="space-y-4 pb-24">
    ...
    {[{ label: "Impayé sur la période", value: Number(metrics?.period_outstanding ?? 0) }].map(item => <div key={item.label} className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3"><div><p className="text-xs font-bold text-muted-foreground">{item.label}</p><p className="mt-0.5 text-[11px] text-muted-foreground">Créances liées aux factures de la période sélectionnée</p></div><p className={`text-base font-black ${item.value > 0.005 ? "text-red-600" : "text-muted-foreground"}`}>{fmt(item.value)}</p></div>)}
    ...
    <ClientReportSection report={clientReport} color={RC} />
    ...
    <div className="bg-card rounded-2xl border border-border p-4 flex items-center justify-between gap-3"><div><p className="font-bold text-sm">Export PDF</p><p className="text-xs text-muted-foreground">KPIs, produits, employés, clients, stock/inventaire et paiements</p></div><button type="button" onClick={() => void downloadPdf()} disabled={exporting || !metrics} className="rounded-xl px-4 py-2.5 text-xs font-black text-white disabled:opacity-50 flex items-center gap-2" style={{ background: RC }}><Download size={15} />{exporting ? "Génération…" : "Télécharger"}</button></div>
  </div>;
}
