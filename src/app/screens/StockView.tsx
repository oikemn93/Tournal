  ...
  async function submitEntry() {
    ...
    try {
      ...
      const receiptCharge = persisted.charge_id ? {
        ...
        fournisseur:supplier.nom, supplierId:supplier.id, status:Number(dMontant)>0 ? "pending" as const : "paid" as const, paidAmount:0,
  ...
  async function submitNew() {
    ...
    try {
      ...
      const receiptCharge = movement?.charge_id && supplier ? {
        ...
        date:today(), dateRaw:new Date().toISOString(), categorie:"Achat stock" as const, recurrence:"unique" as const,
        fournisseur:supplier.nom, supplierId:supplier.id, status:Number(nMontant)>0 ? "pending" as const : "paid" as const, paidAmount:0,
