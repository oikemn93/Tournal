...
ok(ui.includes('aria-label="Catégories du rapport"')&&ui.includes('role="tablist"')&&ui.includes('role="tabpanel"'),"professional tab navigation missing");
ok(ui.includes('data-report-active-section')&&!ui.includes('function Accordion('),"report details must render as one active panel, not stacked accordions");
