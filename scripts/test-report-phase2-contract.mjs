...
assert(report.includes('...(canSeeMargin?') && report.includes('id:"team"') && report.includes('open==="team"&&canSeeMargin&&body("team"'), "Employee performance UI must stay margin-permission gated");
assert(/section\s*===\s*"team"/.test(report) && report.includes("loadEmployeePerformanceReport"), "Employee report must lazy-load when the team section opens");
