...
# Keep synthetic object construction harmlessly for now, but never merge it into UI stock.
s=s.replace('          ...(stockEntries.length ? { entries:[...entries, ...stockEntries] } : {}),\n','',1)
p.write_text(s)

# 3) POS express: backend/realtime is the only stock authority.
p=Path('src/app/screens/POSView.tsx'); s=p.read_text()
s=s.replace('        ...(saleEntries.length ? { entries:[...entries, ...saleEntries] } : {}),\n','',1)
p.write_text(s)

# 4) Factures: same rule for encaissement, creation-with-payment and returns.
p=Path('src/app/screens/FacturesView.tsx'); s=p.read_text()
s=s.replace('      ...(saleEntries.length ? { entries: [...entries, ...saleEntries] } : {}),\n','',1)
s=s.replace('      ...(saleEntries.length ? { entries:[...entries,...saleEntries] } : {}),\n','',1)
s=s.replace('    onUpdate({ invoices: [...invoices, retInv], entries: [...entries, ...restoreEntries] });','    // return_sale already persisted the stock restoration; realtime refreshes entries.\n    onUpdate({ invoices: [...invoices, retInv] });',1)
p.write_text(s)
