...
-- Definitions below are intentionally byte-aligned with pg_get_functiondef in production.

CREATE OR REPLACE FUNCTION private.fifo_realized_margin_core(p_boutique_id text, p_from_at timestamp with time zone, p_to_at timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.get_fifo_realized_margin(p_boutique_id text, p_from_at timestamp with time zone, p_to_at timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
...
CREATE OR REPLACE FUNCTION public.get_financial_metrics(p_boutique_id text, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
