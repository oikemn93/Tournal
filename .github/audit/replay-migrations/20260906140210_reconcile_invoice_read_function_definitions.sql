-- Audit-only replay reconciliation.
-- Production was applied through the Supabase migration API, whose stored
-- PL/pgSQL source formatting differs from the compact replay source. The
-- semantics, ACLs and function attributes are unchanged; these definitions
-- reproduce the exact production function bodies so the structural fingerprint
-- remains byte-for-byte deterministic.

CREATE OR REPLACE FUNCTION private.read_bounded_invoice_summaries(p_boutique_id text, p_from timestamp with time zone, p_to timestamp with time zone, p_include_pending boolean)
 RETURNS SETOF jsonb
 LANGUAGE plpgsql
  ...
  if p_include_pending then
    return query
      with filtered as materialized (
        ...
        join filtered f
          on f.boutique_id = l.boutique_id
         and f.id = l.invoice_id
...
CREATE OR REPLACE FUNCTION private.read_invoice_lines(p_boutique_id text, p_invoice_ids text[])
 RETURNS SETOF jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
declare
  ...
  v_ids text[];
begin
  if p_boutique_id is null or p_invoice_ids is null or cardinality(p_invoice_ids) = 0 then
    return;
  end if;
