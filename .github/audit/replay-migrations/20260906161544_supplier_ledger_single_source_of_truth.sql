...
as $function$
  select s.id,
         coalesce((
           select sum(greatest(c.montant - c.paid_amount, 0::numeric))
...
begin
  ...
  select coalesce(sum(greatest(c.montant - c.paid_amount, 0::numeric)), 0)
  into v_due
  from public.charges c
  where c.boutique_id = p_boutique_id
  ...
  if v_remaining > 0 then
    raise exception 'supplier ledger inconsistent';
  end if;
...
begin
  ...
  if p_supplier_id is not null and p_type = 'achat' and p_qty > 0 and not v_is_self_supplier then
    ...
    v_charge_id := nextval('private.charge_id_seq');
    v_receipt_amount := p_qty * coalesce(p_prix_unit, 0);
    ...
    insert into public.charges (
      ...
      v_receipt_amount, 'Achat stock', now(), v_user,
      nullif(trim(p_note), ''), v_supplier.nom, p_supplier_id,
      case when v_receipt_amount = 0 then 'paid' else 'pending' end,
      0, 'supplier_receipt', v_entry_id, v_due_date
    );
  end if;
...
begin
  ...
  if v_product.stock + v_delta_qty < 0 then
    raise exception 'receipt correction would make stock negative';
  end if;
...
begin
  ...
  if v_can_margin then
    select coalesce(sum(case when lower(trim(coalesce(i.type,'')))='retour' then -1 else 1 end*((coalesce(l.sell_qty,l.qty)*l.prix_unit)-(l.qty*coalesce(l.prix_achat,0)))),0)
    into v_margin
    from public.invoices i
...
update public.charges
set status = case
  when montant = 0 or paid_amount >= montant then 'paid'
  when paid_amount > 0 then 'partial'
  else 'pending'
end,
updated_at = now()
where source = 'supplier_receipt'
