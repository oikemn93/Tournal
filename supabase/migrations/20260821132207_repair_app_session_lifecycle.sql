-- Keep one application session per Supabase Auth session, not per user. A
-- cashier may legitimately use a phone and a workstation at the same time.
-- Locks remain bound to their own Auth session and cannot be bypassed by a
-- background renewal using the same JWT.

do $$
begin
  if exists (select 1 from private.app_sessions where session_id is null) then
    raise exception 'app_sessions.session_id must be backfilled before this migration';
  end if;
end;
$$;

alter table private.app_sessions alter column session_id set not null;
alter table private.app_sessions drop constraint app_sessions_pkey;
...
begin
  ...
  where user_id = v_user and boutique_id = p_boutique_id and session_id = v_sid;
  ...
  return jsonb_build_object('expires_at', v_expires, 'locked', coalesce(v_locked, false));
end;
$$;
...
begin
  ...
  where user_id = v_uid and boutique_id = p_boutique_id and session_id = v_sid;
end;
$$;
