-- Harden future returns without rewriting uncertain historical rows.
-- Existing NOT VALID constraints continue to tolerate legacy incomplete returns,
-- while this trigger requires every new return line to belong to the exact
-- source invoice and serializes quantity checks per source line.
...
before insert or update of boutique_id, invoice_id, source_invoice_line_id, product_id, qty
