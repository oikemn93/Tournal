alter table public.stock_transfer_lines add column if not exists sell_unit text;
alter table public.stock_transfer_lines add column if not exists sell_qty numeric;

alter table public.stock_transfer_lines drop constraint if exists stock_transfer_lines_sell_qty_check;
alter table public.stock_transfer_lines add constraint stock_transfer_lines_sell_qty_check
  check (sell_qty is null or sell_qty > 0);
...
declare
  ...
  v_qty numeric; v_sell_qty numeric; v_sell_unit text; v_price numeric; v_discount numeric; v_total numeric:=0; v_response jsonb;
...
begin
  ...
  for v_line in select value from jsonb_array_elements(p_lines) loop
    ...
    insert into public.stock_transfer_lines(transfer_id,source_boutique_id,source_product_id,product_name,unit,qty,prix_unit,discount_percent,sell_unit,sell_qty)
    values(v_transfer,p_from_boutique_id,v_product.id,v_product.nom,v_product.unit,v_qty,v_price,v_discount,v_sell_unit,case when v_sell_unit is null then null else v_sell_qty end);
    v_total:=v_total+v_sell_qty*v_price*(1-v_discount/100);
  end loop;
...
begin
  ...
  for v_line in select * from public.stock_transfer_lines where transfer_id=p_transfer_id order by source_product_id,id loop
    ...
    v_line_total:=coalesce(v_line.sell_qty,v_line.qty)*v_line.prix_unit*(1-v_line.discount_percent/100);
  ...
  if v_transfer.relationship_type='commercial' then
    ...
    select v_transfer.from_boutique_id,v_invoice_id,stl.source_product_id,stl.product_name,stl.qty,stl.unit,
           stl.prix_unit*(1-stl.discount_percent/100),coalesce(stl.source_unit_cost,0),stl.sell_unit,stl.sell_qty
