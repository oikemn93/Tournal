...
begin
  ...
  from public.auth_settings where boutique_id=p_boutique_id;
...
create policy "notifications: active boutique select"
...
alter table public.push_subscriptions
  add column if not exists boutique_id text references public.boutiques(id) on delete cascade;

update public.push_subscriptions s
set boutique_id=c.boutique_id
from private.notification_session_context c
where s.user_id=c.user_id and s.boutique_id is null;
...
create index if not exists push_subscriptions_user_boutique_enabled_idx
  on public.push_subscriptions(user_id,boutique_id,enabled);
